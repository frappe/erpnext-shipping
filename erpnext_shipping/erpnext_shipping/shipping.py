...
from frappe.utils import Any
import frappe
from erpnext.stock.doctype.shipment.shipment import get_company_contact
from erpnext_shipping.erpnext_shipping.doctype.shiprocket.constants import SHIPROCKET_PROVIDER, status_map
...
from erpnext_shipping.erpnext_shipping.doctype.shiprocket.shiprocket import ShiprocketUtils
...
def fetch_shipping_rates(
	...
	if shiprocket_enabled:
		...
		shiprocket_prices = (
			shiprocket.get_available_services(
				delivery_address=delivery_address,
				pickup_address=pickup_address,
				parcels=parcels,
				description_of_content=description_of_content,
...
def create_shipment(
	...
	if service_info["service_provider"] == SHIPROCKET_PROVIDER:
		...
		shipment_info = shiprocket.create_shiprocket_shipment(
			...
			delivery_company_name=delivery_company_name,
			service_info=service_info,
			shipment_parcel=json.dumps(shipment_parcel),
			description_of_content=description_of_content,
			value_of_goods=value_of_goods,
			total_weight=sum(
				float(p.get("weight", 0)) * int(p.get("count", 1))
				for p in (shipment_parcel if isinstance(shipment_parcel, list) else [shipment_parcel])
			),
		)
...
def print_shipping_label(shipment: str):
	...
	if service_provider == LETMESHIP_PROVIDER:
		...
		shipping_label = []
		shiprocket = ShiprocketUtils(company=pickup_company)
		label_url = shiprocket.get_label(shipment_id=shipment_id)
		if not label_url:
