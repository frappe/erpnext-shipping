...
class ShiprocketUtils:
	def __init__(self, company):
		doc = get_shipping_provider(company, "Shiprocket")
		self.barer_key = doc["barer_key"]
	...
	def get_available_services(self, parcels, delivery_address_name, pickup_address_name, total_weight):
		...
		if response.status_code == 200:
			...
			for services in services_available:
				...
				available_service = self.get_service_dict(services, parcels, token)
	...
	def get_service_dict(self, service, parcels: list[dict], token):
	...
	def create_shiprocket_shipment(
		...
		if not (shipment, pickup_company, total_weight):
			return
		pickup_location_id = get_pickup_location(pickup_company, "Shiprocket")

		if not pickup_location_id:
			frappe.throw("Unable to find Pickup Location for the given company.")

		mentiond_pickup_address = self.get_shiprocket_shipping_address(pickup_location_id)
		...
		if response.status_code == 200:
			...
			if shipment_id:
				ship_now_url = "https://apiv2.shiprocket.in/v1/external/courier/assign/awb"
				ship_now_payload = {
				...
				ship_now_response = requests.post(
					ship_now_url, json=ship_now_payload, headers=ship_now_headers
				)
	...
	def generate_lable(self, shipment_id):
		bearer_key = self.barer_key

		url = "https://apiv2.shiprocket.in/v1/external/courier/generate/label"

		payload = json.dumps({"shipment_id": shipment_id})
		headers = {"Content-Type": "application/json", "Authorization": f"Bearer {bearer_key}"}
